...
title Windows Defender Manager - Educational Use Only
...
:disable_complete
echo.
echo [STEP 1/6] Checking Tamper Protection...
powershell -Command "Get-MpPreference | Select-Object DisableTamperProtection" | findstr "False" >nul && (
    echo [WARNING] Tamper Protection is enabled! Use option 6 first.
    timeout /t 3
    goto main_menu
)

echo [STEP 2/6] Disabling via Group Policy...
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows Defender" /v DisableAntiSpyware /t REG_DWORD /d 1 /f >nul
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows Defender" /v ServiceKeepAlive /t REG_DWORD /d 0 /f >nul

echo [STEP 3/6] Configuring Real-time Protection registry...
...
echo [STEP 4/6] Applying PowerShell configurations...
powershell -Command "Set-MpPreference -DisableRealtimeMonitoring $true -DisableBehaviorMonitoring $true -DisableIOAVProtection $true -DisablePrivacyMode $true -DisableBlockAtFirstSeen $true -DisableScriptScanning $true -DisableArchiveScanning $true -DisableCatchupFullScan $true -DisableCatchupQuickScan $true -ErrorAction SilentlyContinue"

echo [STEP 5/6] Stopping and disabling services...
sc stop %defenderService% >nul 2>&1
sc config %defenderService% start= disabled >nul
sc stop "WdNisSvc" >nul 2>&1
sc config "WdNisSvc" start= disabled >nul
sc stop "Sense" >nul 2>&1
sc config "Sense" start= disabled >nul

echo [STEP 6/6] Killing running processes...
...
echo [%date% %time%] Defender completely disabled >> "%logFile%"
timeout /t 3
goto main_menu
...
powershell -Command "Set-MpPreference -DisableRealtimeMonitoring $false -DisableBehaviorMonitoring $false -DisableIOAVProtection $false -DisableBlockAtFirstSeen $false -DisableScriptScanning $false -DisableArchiveScanning $false -ErrorAction SilentlyContinue"

echo [STEP 3/4] Enabling and starting services...
sc config %defenderService% start= auto >nul
...
powershell -Command "Remove-MpPreference -ExclusionPath (Get-MpPreference).ExclusionPath -ErrorAction SilentlyContinue"

echo [SUCCESS] Windows Defender enabled and reset!
echo [%date% %time%] Defender enabled >> "%logFile%"
timeout /t 3
goto main_menu
...
if "%exclusionPath%"=="" goto main_menu

if not exist "%exclusionPath%" (
    echo [WARNING] Path does not exist! Create it? (Y/N)
    set /p "createPath="
    if /i "!createPath!"=="Y" (
...
powershell -Command "Add-MpPreference -ExclusionPath '%exclusionPath%' -ErrorAction SilentlyContinue"
if errorlevel 1 (
    echo [ERROR] Failed to add exclusion!
) else (
    echo [SUCCESS] Added exclusion: %exclusionPath%
    echo [%date% %time%] Exclusion added: %exclusionPath% >> "%logFile%"
)
timeout /t 2
goto main_menu
...
echo [%date% %time%] All exclusions removed >> "%logFile%"
timeout /t 2
goto main_menu
...
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows Defender Security Center\App and Browser protection" /v DisallowWindowsSecurityAppBrowserProtection /t REG_DWORD /d 1 /f >nul
...
echo [%date% %time%] Tamper Protection bypass attempted >> "%logFile%"
timeout /t 2
goto main_menu
...
if "%serviceChoice%"=="1" (
    sc stop %defenderService%
    sc stop "WdNisSvc"
    sc stop "Sense"
    echo Services stopped!
) else if "%serviceChoice%"=="2" (
    sc start %defenderService%
    sc start "WdNisSvc" 
    sc start "Sense"
    echo Services started!
) else if "%serviceChoice%"=="3" (
    sc config %defenderService% start= disabled
    sc config "WdNisSvc" start= disabled
    sc config "Sense" start= disabled
    echo Services set to disabled!
) else if "%serviceChoice%"=="4" (
    sc config %defenderService% start= auto
    sc config "WdNisSvc" start= auto
    sc config "Sense" start= auto
    echo Services set to automatic!
)
if not "%serviceChoice%"=="5" (
    echo [%date% %time%] Service management: Option %serviceChoice% >> "%logFile%"
    timeout /t 2
)
goto main_menu
...
powershell -Command "Checkpoint-Computer -Description 'Pre-Defender-Disabled-Educational' -RestorePointType 'MODIFY_SETTINGS'" >nul 2>&1
if errorlevel 1 (
...
powershell -Command "Get-MpComputerStatus | Select-Object AntivirusEnabled, RealTimeProtectionEnabled, AntispywareEnabled" | findstr "True" >nul && set "defenderEnabled=1"

if %defenderEnabled%==1 (
    echo [STATUS: ENABLED] - Defender is active
) else (
    echo [STATUS: DISABLED] - Defender is inactive
)
goto :eof

:exit_script
echo.
echo [%date% %time%] Script exited by user >> "%logFile%"
echo Thank you for using Advanced Defender Manager
